...
import { TIME_CONSTANTS, Settings } from '../lib/utils';
...
export class EventLayoutManager {
    ...
    /**
     * 計算されたレイアウトグループの配列
     * @type {Array<Array<EventData>>}
     * @private
     */
    private layoutGroups: EventData[][];
     ...
     * @private
     */
    private _timeCache: Map<string, number>;
    ...
    constructor() {
         ...
         * @private
         */
        this._timeCache = new Map<string, number>();
    }
     ...
     */
    private _getTimeInMillis(time: Date | number, id: string, type: string): number {
        // キャッシュキーを生成
        const cacheKey = `${id}_${type}`;

        // キャッシュに存在する場合はそれを返す
        if (this._timeCache.has(cacheKey)) {
            return this._timeCache.get(cacheKey) as number;
        }
     ...
     * 
     * @param {EventData} eventData - イベント情報
    ...
    clearEvents(): void {
        ...
        this.events.forEach(event => {
            if (event && event.element) {
                // イベント要素への参照を解放
                event.element = null as unknown as HTMLElement;
            }
        });
     ...
     * 
     * @private
     * @returns {Array<Array<EventData>>} 重なるイベントのグループ配列
     */
    private _groupOverlappingEvents(): EventData[][] {
     ...
     * 
     * @private
     * @param {EventData[]} events - イベントの配列
     * @returns {number[]} - 各イベントに対応するレーン番号（インデックス）
     */
    private _assignLanes(events: EventData[]): number[] {
        ...
        const lanes: number[] = [];  // 各レーンの最終終了時刻を記録
...
export class TimeTableManager {
     ...
     * 業務時間の長さ（時間単位）
     */
    private hourDiff: number;

    /**
     * 現在時刻を示す線のHTML要素
     */
    private currentTimeLine: HTMLElement | null;
     ...
     */
    constructor(parentDiv: HTMLElement, baseDiv: HTMLElement) {
        /**
         * タイムテーブルを格納する親要素
         */
     ...
     * 
     * @param {Settings} settings - 時間設定オブジェクト
     ...
     */
    initializeTimeVariables(): { openTimeHour: number, openTimeMinute: number } {
     ...
     */
    createBaseTable(breakTimeFixed: boolean, breakTimeStart: string = '12:00', breakTimeEnd: string = '13:00'): void {
     ...
     * @returns {void}
     */
    private _createWorkTimeWithBreak(breakTimeStart: string, breakTimeEnd: string, unitHeight: number): void {
     ...
     * @returns {void}
     */
    private _createWorkTimeWithoutBreak(unitHeight: number): void {
     ...
     * @returns {void}
     */
    private _addTimeLabelsAndLines(openTimeMinute: number, unitHeight: number): void {
