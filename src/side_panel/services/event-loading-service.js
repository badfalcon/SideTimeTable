...
export class EventLoadingService {
     ...
     * @param {Object} [deps.outlookEventManager]
     */
    setDeps(deps) {
        this._deps = deps;
    }
    ...
    async loadEventsForDate(targetDate) {
        ...
        const { allDayEventsComponent, timelineComponent, eventLayoutManager,
                localEventManager, googleEventManager, outlookEventManager } = this._deps;
        ...
        try {
            ...
            // Load Google, Outlook, and local events in parallel
            const loadTasks = [
                localEventManager.loadLocalEvents(targetDate),
                googleEventManager.fetchEvents(targetDate)
            ];
            if (outlookEventManager) {
                loadTasks.push(outlookEventManager.fetchEvents(targetDate));
            }
