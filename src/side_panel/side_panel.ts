...
class UIController {
    ...
    private eventLayoutManager: EventLayoutManager | null;
    private googleEventManager: GoogleEventManager | null;
    private localEventManager: LocalEventManager | null;
    ...
    initialize(): void {
        ...
        if (!parentDiv || !baseDiv || !googleEventsDiv || !localEventsDiv) {
     ...
     * @private
     */
    private _initializeTimeList(): void {
     ...
     * @private
     */
    private _setupDialogElements(): void {
        // ローカルイベントダイアログ要素
        ...
        if (!localEventDialog || !eventTitleInput || !eventStartTimeInput || 
            !eventEndTimeInput || !saveEventButton || !deleteEventButton || !this.localEventManager) {
        ...
        if (!alertModal || !alertMessage || !closeAlertButton || !this.localEventManager) {
            ...
            .then(settings => {
                try {
                    ...
                    setTimeout(() => {
                        ...
                        // Googleイベントの取得
                        if (settings.googleIntegrated && this.googleEventManager) {
                            console.log('Googleイベント取得開始');
                            this.googleEventManager.fetchEvents();
                        } else if (this.eventLayoutManager) {
                    ...
                    logError('設定適用', errorMessage);
                }
            })
            .catch(error => {
                const errorMessage = error instanceof Error ? error : String(error);
                logError('設定読み込み', errorMessage);
            });
    }
    ...
    private _setupEventListeners(): void {
        ...
        const eventTitleInput = document.getElementById('eventTitle') as HTMLInputElement | null;
        const eventStartTimeInput = document.getElementById('eventStartTime') as HTMLInputElement | null;
        const eventEndTimeInput = document.getElementById('eventEndTime') as HTMLInputElement | null;

        if (!addLocalEventButton || !localEventDialog || !closeDialog || 
            !saveEventButton || !cancelEventButton || !eventTitleInput || 
            !eventStartTimeInput || !eventEndTimeInput || !this.localEventManager) {
            console.error('ローカルイベント関連の要素が見つかりません');
            return;
        }
        ...
        if (!alertModal) {
            console.error('アラートモーダルが見つかりません');
            return;
        }
     ...
     * @private
     */
    private _setupPeriodicUpdates(): void {
