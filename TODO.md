...
- [ ] 終日イベント（`start.date`/`end.date`）の作成対応。
- [ ] 作成した Google イベントの編集・削除（現状は作成のみ。編集/削除は Google カレンダー Web 側で行う導線）。
- [ ] `background.js` の `createEvent` ハンドラ自体の単体テスト（現状はクライアント層 `createEvent` のテストでカバー。ハンドラ専用テストの前例がないため未整備）。
